...
Extract 16 frames from videos in training_set, use YOLO to detect persons, expand to 384x384 centered on detection box and crop
"""
...
class VideoPreprocessor:
    ...
    def __init__(self, 
        ...
        Args:
            training_set_dir: Training set directory
            output_dir: Output directory
            num_frames: Number of frames to extract per video
            crop_size: Crop region size (square side length)
        """
        self.training_set_dir = Path(training_set_dir)
    ...
    def process_all_videos(self):
        """Process all videos"""
        video_files = list(self.training_set_dir.glob("*.avi"))
        if not video_files:
            print(f"Error: No video files found in {self.training_set_dir}")
            return
...
def main():
    parser = argparse.ArgumentParser(description='Video preprocessing script - Use YOLO to detect persons and crop to 384x384')
    parser.add_argument('--training_set_dir', type=str, default='training_set',
                        help='Training set directory')
    parser.add_argument('--output_dir', type=str, default='preprocessed_frames',
