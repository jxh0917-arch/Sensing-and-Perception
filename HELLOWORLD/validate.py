"""
Validation script
Evaluate model on validation set and generate detailed report
...
def validate_model(model_path: str = None, save_results: bool = True):
    """
    ...
    with torch.no_grad():
        for frames, arm_images_list, labels in tqdm(val_loader, desc='Validating'):
    ...
    if save_results:
        ...
        report_path = Config.RESULTS_DIR / 'validation_report.txt'
        utils.save_evaluation_report(results, str(report_path), class_names)
        
        # Save confusion matrix
        cm_path = Config.RESULTS_DIR / 'validation_confusion_matrix.png'
        utils.plot_confusion_matrix(results['confusion_matrix'], class_names, str(cm_path))
        
        # Save prediction results
        results_path = Config.RESULTS_DIR / 'validation_predictions.npz'
        np.savez(
            results_path,
...
def main():
    ...
    validate_model(model_path=args.model, save_results=not args.no_save)
